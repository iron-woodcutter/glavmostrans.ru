...
add_action( 'wp_enqueue_scripts',             'theme26118_include_custom_assets', 11 );

// Print a `totop` button on frontend.
add_action( 'cherry_footer_after',            'theme26118_print_totop_button' );

// Adds a new theme option - `totop` button.
add_filter( 'cherry_general_options_list',    'theme26118_add_totop_option' );

// Adds a new theme option - `Google Analytics Code`.
add_filter( 'cherry_general_options_list',    'theme26118_add_google_code' );

// Print a google analytics code on the bottom of HTML document.
add_filter( 'wp_footer',                      'theme26118_print_google_code', 9999 );

// Modify a comment form.
add_filter( 'comment_form_defaults',          'theme26118_modify_comment_form' );

// Modify the columns on the `Posts` and `Pages` screen.
add_filter( 'manage_posts_columns',           'theme26118_add_thumbnail_column_header' );
add_filter( 'manage_pages_columns',           'theme26118_add_thumbnail_column_header' );
add_action( 'manage_posts_custom_column' ,    'theme26118_add_thumbnail_column_data', 10, 2 );
add_action( 'manage_pages_custom_column' ,    'theme26118_add_thumbnail_column_data', 10, 2 );
 ...
 * @ignore
 */
function theme26118_include_custom_assets() {
	...
	wp_enqueue_style( 'material-icon', ''. get_stylesheet_directory_uri() .'/assets/css/material-design.css', false, '1.0', 'all' );
 ...
 * @ignore
 */
function theme26118_print_totop_button() {
 ...
 * @return array
 */
function theme26118_add_totop_option( $args ) {
 ...
 * @return array
 */
function theme26118_modify_comment_form( $args ) {
	...
	$args['title_reply'] = __( 'leave a <span>reply</span>', 'child-theme-domain' );

	$args['title_reply_to'] = __( 'Leave reply to <span>%s</span>', 'child-theme-domain' );

	$args['fields']['author'] = '<div class="cf_left"><p class="comment-form-author"><input id="author" name="author" type="text" placeholder="' . __( 'Name', 'child-theme-domain' ) . '" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . $html_req . ' />';
	...
	$args['fields']['url'] = '<p class="comment-form-url"><input id="url" name="url" ' . ( $html5 ? 'type="url"' : 'type="text"' ) . ' placeholder="' . __( 'Website', 'child-theme-domain' ) . '" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></p></div>';

	$args['comment_field'] = '<div class="cf_right"><p class="comment-form-comment"><textarea id="comment" name="comment" placeholder="' . __( 'Comment', 'child-theme-domain' ) . '" cols="45" rows="8" aria-describedby="form-allowed-tags" aria-required="true" required="required"></textarea></p></div>';
 ...
 * @return array
 */
function theme26118_add_thumbnail_column_header( $post_columns ) {
 ...
 */
function theme26118_add_thumbnail_column_data( $column, $post_id ) {
 ...
 * @return array
 */
function theme26118_add_google_code( $options ) {
 ...
 * @ignore
 */
function theme26118_print_google_code() {
...
add_filter( 'cherry_custom_font_icons', 'theme26118_custom_font_icons' );

function theme26118_custom_font_icons($icons) {
	$icons['theme26118_material_icon'] = get_stylesheet_directory_uri() . '/assets/css/material-design.css';
		return $icons;
}


add_action( 'init', 'theme26118_replace_breadcrumbs' );
function theme26118_replace_breadcrumbs() {
	remove_action( 'cherry_content_before', 'cherry_get_breadcrumbs', 5 );
	add_action( 'cherry_header', 'cherry_get_breadcrumbs', 99 );
}

add_filter( 'cherry_breadcrumbs_custom_args', 'theme26118_breadcrumbs_wrapper_format' );
function theme26118_breadcrumbs_wrapper_format( $args ) {
...
add_filter( 'cherry_breadcrumbs_options_list', 'theme26118_breadcrumbs_options_list' );
function theme26118_breadcrumbs_options_list( $breadcrumbs_options ) {
...
add_filter( 'cherry_css_var_list', 'theme26118_add_css_var' );
function theme26118_add_css_var( $var_list ) {
	$var_list[] = 'breadcrumbs-background';

	return $var_list;
}

 /*** remove related posts title ***/

add_filter('cherry_related_posts_output_args', 'theme26118_related_posts_output_args');
function theme26118_related_posts_output_args($default_args){

  $default_args['format_title'] = '';
    
    return $default_args;
}

// Related rebuild
add_filter( 'cherry_related_posts_args', 'theme26118_related_posts_args', 11 );

function theme26118_related_posts_args( $args ) {
	$args['num'] = 3;
...
/* comments text title */
add_filter('cherry_title_comments', 'theme3678_cherry_title_comments');

function theme3678_cherry_title_comments($arg){
 $title_comments = __( 'Leave your <span>comments</span>', 'child-theme-domain');

 return $arg = '<h2 class="comments-title">'. $title_comments .'</h2>';
}

/* change team archive */
add_filter('cherry_the_team_args', 'theme3678_cherry_the_team_args');

function theme3678_cherry_the_team_args($args){
if ( is_post_type_archive( CHERRY_TEAM_NAME ) || is_tax( 'group' ) ) {
	 $args['col_md'] = 6;
	 $args['template'] = 'team_2.tmpl';
	 $args['size'] = 'cherry-thumb-team';
}
